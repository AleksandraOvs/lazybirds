jQuery(document).ready(function ($) {
	...
	var type  = "ADDRESS";
	var $region = $("#billing_region");
	var $city   = $("#billing_city_1, #billing_city");
	var $street = $("#billing_street");
	var $house  = $("#billing_house");

	///php_vars.dadata_suggest_token
	$region.suggestions({
		token: php_vars.dadata_suggest_token,
		type: type,
		hint: false,
	...
	$house.suggestions({
		...
		onSelect: function(suggestion) {
			...
			let ncity = suggestion.data.city_with_type + ', ' + suggestion.data.region_with_type;
			let address = suggestion.data.street_with_type + ', ' + suggestion.data.house;

			$("#billing_city").html('<option value="'+ncity+'">'+ncity+'</option>');
			$("#billing_city").val( ncity );
			///billing_address_1
			$("#billing_address_1").val( ncity );
        }
	});
