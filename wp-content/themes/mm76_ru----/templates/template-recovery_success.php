<?php
/*
Template Name: Recovery Success
*/
...
if ($user_ID) {
   ...
   header( 'Location:' . home_url() );
    ...
    $pre_user = $wpdb->get_row( 
        ...
        FROM " . $wpdb->prefix . "pre_pass
        WHERE hash = '".$_GET['hash']."' "
    );

    if( !$pre_user ) wp_die('Ошибка доступа. Возможно, ссылка для восстановление пароля больше не активна.');
		...
		if(0 === count($errors)) {
            ...
            $wpdb->query(
                $wpdb->prepare(
                    ...
                    SET user_pass = MD5('".$password_new."')
                    WHERE user_email = '%s'",
                    $pre_user->login_email
                )
            );

            $user = get_user_by('email', $pre_user->login_email );

            $wpdb->query(
                $wpdb->prepare(
                    "DELETE FROM " . $wpdb->prefix . "pre_pass
                    WHERE id = '%d'",
                    $pre_user->id
                )
            );
